...
export default function AdminStaffDates() {
  ...
  const close = () => { if (!busy && window.confirm('Закрыть окно без сохранения?')) setDraft(null); };
  ...
  return <main className='admin-module'><div className='admin-module__container'>
    <section className='admin-module__hero'><div><span className='admin-module__badge'>Сотрудники</span><h1>Важные даты</h1></div><Button className='admin-module__button admin-module__button--ghost' startIcon={<ArrowBack />} onClick={() => navigate('/admin/schedule')}>Назад</Button></section>
