...
const AdminRooms = () => {
  ...
  const [subjectsDialog, setSubjectsDialog] = useState({ open: false, room: null });
  ...
  const saveRoom = async () => {
    ...
    try {
      const data = { name: form.name.trim(), is_active: form.is_active };
      if (editDialog.item) {
  ...
  const saveSubjects = async () => {
    try {
      await scheduleService.updateRoomSubjects(subjectsDialog.room.id, selectedSubjectIds);
      ...
      <div className="admin-module__container">
                    ...
                    .map((r) => (
                      <TableRow key={r.id}>
                        ...
                        <TableCell align="center">
                          <IconButton onClick={() => openEdit(r)} size="small" title="Редактировать">
                            <EditIcon />
                          </IconButton>
                          <IconButton onClick={() => openSubjects(r)} size="small" color="primary" title="Предметы кабинета">
                            <SubjectsIcon />
                          </IconButton>
                  ...
                  {!rooms.length && (
                    <TableRow>
                      <TableCell colSpan={4} align="center">
                        <Typography color="text.secondary">Кабинеты не найдены</Typography>
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
        ...
        <Dialog open={editDialog.open} onClose={closeEdit} maxWidth="sm" fullWidth PaperProps={{ className: "admin-module-dialog" }}>
          ...
          <DialogActions className="admin-module-dialog__actions">
            ...
            <Button onClick={saveRoom} variant="contained">Сохранить</Button>
          </DialogActions>
        </Dialog>

        {/* Subjects Dialog */}
        <Dialog open={subjectsDialog.open} onClose={closeSubjects} maxWidth="sm" fullWidth PaperProps={{ className: "admin-module-dialog" }}>
          <DialogTitle className="admin-module-dialog__title">
          ...
          <DialogActions className="admin-module-dialog__actions">
            ...
            <Button onClick={saveSubjects} variant="contained">Сохранить</Button>
          </DialogActions>
        </Dialog>
