...
type ScheduleSlotTeacher struct {
	ID             uint      `json:"id" gorm:"primaryKey"`
	ScheduleSlotID uint      `json:"schedule_slot_id" gorm:"not null;index;uniqueIndex:idx_schedule_slot_teacher"`
	TeacherID      uint      `json:"teacher_id" gorm:"not null;index;uniqueIndex:idx_schedule_slot_teacher"`
	CreatedAt      time.Time `json:"created_at"`

	ScheduleSlot ScheduleSlot `json:"-" gorm:"foreignKey:ScheduleSlotID;constraint:OnUpdate:CASCADE,OnDelete:CASCADE;"`
